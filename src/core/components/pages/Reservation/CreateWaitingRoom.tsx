...
function CreateWaitingRoom(props: CreateWaitingRoomProps) {
  ...
  const postDataToBackend = async (data: WaitingRoomData) => {
    ...
    console.log(newData)
    await axios
      .post("http://localhost:3000/reservation/createwaitingroom", {
        headers: {
          Authorization: "bearer " + token,
        },
        data: newData,
                      ...
                      <div className="my-0" key={i}>
                        <label className="ml-2">
                          <input
                            ...
                            ref={register}
                            name="time_slot"
                            disabled={!times.includes(JSON.stringify(item)) && quota / 30 <= checkedCount}
                          />
                          {formatTime(item)}
                        </label>
