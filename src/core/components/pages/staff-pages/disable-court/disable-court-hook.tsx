...
export const useRow = (initial: ViewRowProps[] = []) => {
  ...
  const onAddRow = (f: { timeSlotStart: string; timeSlotEnd: number; day: string }) => {
    setRowData((prev) => {
  ...
  const onDeleteRow = (indx: string | number) => {
    setRowData((prev) => {
      const arr = [...prev]
...
export const useTableWithPagination = () => {
  ...
  const onDelete = (id: string | number) => {
    client
      ...
      .catch(() => alert("ไม่สามารถลบได้ กรุณาลองใหม่อีกครั้ง"))
  }
  function jumpUp() {
...
export function withDeletable<P>(Component: ComponentType<P>, F: (id: string | number) => void): React.FC<P> {
