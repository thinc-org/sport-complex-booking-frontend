            ...
            case "Rejected": {
              return (
                <div className="alert alert-danger mt-3" role="alert">
                  ...
                  <h6>{context.is_thai_language ? "ข้อมูลไม่ถูกต้อง" : "Rejected Info:"}</h6> {user.rejected_info.map((data)=> <h6 key={data}> - {data}</h6>)}
          ...
          <div className="mx-auto col-md-6">
            ...
            <form onSubmit={handleSubmit(onSubmit)}>
              <div className="default-mobile-wrapper my-3">
                <h4 className="align-right mb-2">{context.is_thai_language ? "ถาษา" : "Language"}</h4>
              ...
              <div className="default-mobile-wrapper">
                <div className="">
                  {/* START OF THE FORM */}
                  <h4>{context.is_thai_language ? "ข้อมูลผู้ใช้" : "Member Information"}</h4>
                  <div className="row">
                    <div className="col-md-4">
                      ...
                      <select name="prefix" ref={register}  disabled={user.verification_status === "Rejected" && !user.rejected_info.includes('prefix')}>
                        <option value={context.is_thai_language ? "นาย" : "Mr."}>{context.is_thai_language ? "นาย" : "Mr."}</option>
                        <option value={context.is_thai_language ? "นางสาว" : "Ms."}>{context.is_thai_language ? "นางสาว" : "Ms."}</option>
                        <option value={context.is_thai_language ? "นาง" : "Mrs."}>{context.is_thai_language ? "นาง" : "Mrs."}</option>
                      </select>
                      {user.rejected_info.includes('prefix') ? (<p className="input-error" >{context.is_thai_language ? "กรุณาส่งข้อมูลอีกครั้ง" : "Please resubmit this field"}</p>) : (null)}
                    ...
                    <div className="col-md-4">
                      ...
                      <select name="gender" ref={register}  disabled={user.verification_status === "Rejected" && !user.rejected_info.includes('gender')}>
                        <option value={context.is_thai_language ? "ชาย" : "Male"}>{context.is_thai_language ? "ชาย" : "Male"}</option>
                        <option value={context.is_thai_language ? "หญิง" : "Female"}>{context.is_thai_language ? "หญิง" : "Female"}</option>
                        <option value={context.is_thai_language ? "อื่นๆ" : "Other"}>{context.is_thai_language ? "อื่นๆ" : "Other"}</option>
                      </select>
                      {user.rejected_info.includes('gender') ? (<p className="input-error" >{context.is_thai_language ? "กรุณาส่งข้อมูลอีกครั้ง" : "Please resubmit this field"}</p>) : (null)}
                    </div>
                  </div>
                  <hr />
                  ...
                  {user.rejected_info.includes('name_th') ? (<p className="input-error" >{context.is_thai_language ? "กรุณาส่งข้อมูลอีกครั้ง" : "Please resubmit this field"}</p>) : (null)}
                  ...
                  {user.rejected_info.includes('surname_th') ? (<p className="input-error" >{context.is_thai_language ? "กรุณาส่งข้อมูลอีกครั้ง" : "Please resubmit this field"}</p>) : (null)}
                  ...
                  {user.rejected_info.includes('name_en') ? (<p className="input-error" >{context.is_thai_language ? "กรุณาส่งข้อมูลอีกครั้ง" : "Please resubmit this field"}</p>) : (null)}  
                  ...
                  {user.rejected_info.includes('surname_en') ? (<p className="input-error" >{context.is_thai_language ? "กรุณาส่งข้อมูลอีกครั้ง" : "Please resubmit this field"}</p>) : (null)}  
                  ...
                  {user.rejected_info.includes('birthday') ? (<p className="input-error" >{context.is_thai_language ? "กรุณาส่งข้อมูลอีกครั้ง" : "Please resubmit this field"}</p>) : (null)}
                  </div>
                  <hr />
                  ...
                  {user.rejected_info.includes('national_id') ? (<p className="input-error" >{context.is_thai_language ? "กรุณาส่งข้อมูลอีกครั้ง" : "Please resubmit this field"}</p>) : (null)}
                  ...
                  {user.rejected_info.includes('marital_status') ? (<p className="input-error" >{context.is_thai_language ? "กรุณาส่งข้อมูลอีกครั้ง" : "Please resubmit this field"}</p>) : (null)}
                  ...
                  {user.rejected_info.includes('address') ? (<p className="input-error" >{context.is_thai_language ? "กรุณาส่งข้อมูลอีกครั้ง" : "Please resubmit this field"}</p>) : (null)}
                  ...
                  {user.rejected_info.includes('personal_email') ? (<p className="input-error" >{context.is_thai_language ? "กรุณาส่งข้อมูลอีกครั้ง" : "Please resubmit this field"}</p>) : (null)}
                  ...
                  {user.rejected_info.includes('home_phone') ? (<p className="input-error" >{context.is_thai_language ? "กรุณาส่งข้อมูลอีกครั้ง" : "Please resubmit this field"}</p>) : (null)}
                  ...
                  {user.rejected_info.includes('phone') ? (<p className="input-error" >{context.is_thai_language ? "กรุณาส่งข้อมูลอีกครั้ง" : "Please resubmit this field"}</p>) : (null)}
              ...
              <div className="default-mobile-wrapper">
                ...
                <div className="col-md-4">
                  ...
                  {user.rejected_info.includes('contact_person_prefix') ? (<p className="input-error" >{context.is_thai_language ? "กรุณาส่งข้อมูลอีกครั้ง" : "Please resubmit this field"}</p>) : (null)}
                </div>
                ...
                <input name="contact_person.contact_person_name" type="text" ref={register({
                  ...
                  {user.rejected_info.includes('contact_person_name') ? (<p className="input-error" >{context.is_thai_language ? "กรุณาส่งข้อมูลอีกครั้ง" : "Please resubmit this field"}</p>) : (null)}
                ...
                <input name="contact_person.contact_person_home_phone" type="number" ref={register({
                  ...
                  {user.rejected_info.includes('contact_person_home_phone') ? (<p className="input-error" >{context.is_thai_language ? "กรุณาส่งข้อมูลอีกครั้ง" : "Please resubmit this field"}</p>) : (null)}
                ...
                <input name="contact_person.contact_person_phone" type="number" ref={register({
                  ...
                  {user.rejected_info.includes('contact_person_phone') ? (<p className="input-error" >{context.is_thai_language ? "กรุณาส่งข้อมูลอีกครั้ง" : "Please resubmit this field"}</p>) : (null)}
                  {errors.contact_person_phone && <p id="input-error">{errors.contact_person_phone.message}</p>}
                
              </div>
              <br />
              <div className="default-mobile-wrapper">
                <h4>{context.is_thai_language ? "สมาชิก" : "Membership"}</h4>
                <hr />
                ...
                {user.rejected_info.includes('user_photo') ? (<p className="input-error" >{context.is_thai_language ? "กรุณาส่งข้อมูลอีกครั้ง" : "Please resubmit this field"}</p>) : (null)}
                <hr />
                <label className="form-label my-2">
                  {context.is_thai_language ? "หมายเลขบัตรประชาชน / พาสปอร์ต (.pdf เท่านั้น)" : "National ID / Passpord (.pdf only)"}
                </label>
                <div className="form-file">
                ...
                {user.rejected_info.includes('national_id_photo') ? (<p className="input-error" >{context.is_thai_language ? "กรุณาส่งข้อมูลอีกครั้ง" : "Please resubmit this field"}</p>) : (null)}
                <hr />
                ...
                {user.rejected_info.includes('medical_certificate') ? (<p className="input-error" >{context.is_thai_language ? "กรุณาส่งข้อมูลอีกครั้ง" : "Please resubmit this field"}</p>) : (null)}
                <hr />
                <label className="form-label my-2">
                  {is_thai_language
                ...
                {user.rejected_info.includes('house_registration_number') ? (<p className="input-error" >{context.is_thai_language ? "กรุณาส่งข้อมูลอีกครั้ง" : "Please resubmit this field"}</p>) : (null)}
                <hr />
                <label className="form-label my-2">
                  {context.is_thai_language ? "ไม่บังคับ: เอกสารยืนยันตัวตน (.pdf only)" : "Optional: Relationship Verification document (.pdf only)"}
                </label>
                <div className="form-file">
                ...
                {user.rejected_info.includes('relationship_verification_document') ? (<p className="input-error" >{context.is_thai_language ? "กรุณาส่งข้อมูลอีกครั้ง" : "Please resubmit this field"}</p>) : (null)}
              </div>
              <br />
