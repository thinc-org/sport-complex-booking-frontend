...
function ChangePassword() {
  ...
  const postDataToBackend = async (data: PasswordData) => {
    await client
      .post("/account_info/change_password", data)
      .then(() => {
    ...
    <div className="mx-auto col-md-6 mt-3">
      ...
      <div className="default-mobile-wrapper animated-card">
        <div className="row mt-2">
          <div className="col-8">
            <h4 className="align-right">{t("changePassword")}</h4>
          </div>
        </div>
      ...
      <CustomPasswordModal
        ...
        show={showChangeError}
        setShow={setShowChangeError}
        mainFunction={() => setShowChangeError(false)}
      />
      {/* Old Password Mismatch */}
      <CustomPasswordModal
        type={"oldPasswordMismatch"}
        show={showOldPasswordError}
        setShow={setShowOldPasswordError}
        mainFunction={() => setShowOldPasswordError(false)}
      />
    </div>
  )
}
